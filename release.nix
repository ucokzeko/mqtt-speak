...
let
  ...
  buildTools = pkgs.callPackage ./build_scripts/build.nix { doNotBrowsify = ["request" "express"]; doNotDelete = ["audio-files"]; };
in rec {
  inherit (pkg) tarball;
  ...
  integrationTest = ''
    ...
    SPEAK_AUDIO_PATH=/var/lib/mqtt-speak/audio-files/ \
    PREFIX_TONE_FILE=/var/lib/mqtt-speak/audio-files/notify.mp3 \
